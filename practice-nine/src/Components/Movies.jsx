...
const Movies = () => {
  ...
  const getVideoSearchResults = (url, apiKey, searchInput) => {
    axios
      .get(url + "?apikey=" + apiKey + "&s=" + searchInput)
      .then((response) => {
  ...
  return (
      <main className="flex-grow flex flex-col gap-10 m-3">
        <form onSubmit={(e) => e.preventDefault()}>
          <input
            type="text"
            className="w-64 h-8 rounded border border-gray-400 pl-1.5 outline-gray-400"
          ...
          <button
            ...
            onClick={() => getVideoSearchResults(URL, API_KEY, searchInput)}
          >
            Search
          </button>
        </form>
